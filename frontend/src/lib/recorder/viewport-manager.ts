...
export class SimpleViewportManager {
  ...
  constructor(
    ...
    private onZoomChange?: (zoom: number, x?: number, y?: number) => void
  ) {
    this.config = {
      minZoom: 1,
      maxZoom: 3,
  ...
  setZoom(zoom: number) {
    ...
    this.targetTransform.zoom = newZoom;

    // Notify listener of new target
    if (this.onZoomChange) {
      this.onZoomChange(newZoom, this.lastCursorPos?.x, this.lastCursorPos?.y);
    }
